...
// ── Geometry ─────────────────────────────────────────────────────────
// 12 vertices: all permutations of (±1, ±1, ±3)/3
const V = [
  [ 1/3,  1/3,   1],  // 0
  [ 1/3, -1/3,  -1],  // 1
  [-1/3,  1/3,  -1],  // 2
...
// Faces with outward-facing winding
...
function buildFaces() {
  ...
  const hexC = [0.10, 0.10, 0.14];
  const addFace = (idxs, c) => {
    const n = norm(cross(sub(V[idxs[1]], V[idxs[0]]), sub(V[idxs[2]], V[idxs[0]])));
    for (let i = 1; i < idxs.length - 1; i++) {
      for (const v of [V[idxs[0]], V[idxs[i]], V[idxs[i+1]]]) {
        pos.push(...v); nor.push(...n); col.push(...c);
      }
    }
  };
  TRI.forEach(f => addFace(f, triC));
  HEX.forEach(f => addFace(f, hexC));
...
// ── Shaders ───────────────────────────────────────────────────────────
// Face vertex: displacement along normal driven by mouse proximity + wave
const FV = `
...
// Edge vertex: same displacement using normalized position as approximate normal
const EV = `
...
const EF = `
  precision mediump float;
  void main() { gl_FragColor = vec4(0.96, 0.96, 0.96, 0.55); }
`;
...
//   app/components/TruncatedTetraCanvas.js  — shaders (FV/EV), geometry (V/TRI/HEX), animation params
...
export default function TruncatedTetraCanvas() {
  ...
  useEffect(() => {
    ...
    const pBuf = buf(faces.pos);
    const nBuf = buf(faces.nor);
    const cBuf = buf(faces.col);
    ...
    const render = () => {
      ...
      setAttr(fProg, "a_pos", pBuf, 3);
      setAttr(fProg, "a_nor", nBuf, 3);
      setAttr(fProg, "a_col", cBuf, 3);
